...
const zone = Form.create()(React.createClass({
  ...
  dCreator(item, tag) {
    ...
    item.layout = item.layout ? item.layout : formItemLayout
    item.options = item.options ? item.options : zoneOptions
    return (
      <Col span={24} key={item.label}>
      <FormItem {...item.layout} label={item.label}>
        {getFieldDecorator(item.Id, item.options)(tag)}
      </FormItem>
      </Col>
    )
  },
